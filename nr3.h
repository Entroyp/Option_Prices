...
public:
	...
	NRmatrix<T> trans( const NRmatrix<T> &rhs );

	NRmatrix transpose(); 
	NRmatrix operator*(const NRmatrix &rhs);
	NRmatrix operator*(const double &a);
	NRmatrix operator-(const NRmatrix &rhs);
	NRmatrix operator+(const NRmatrix &rhs);
...
#ifdef _CheckDim_
if (mm!=rhs.nrows();) {
	throw("Cannot perform multiplication");
}
#endif
...
template <class T>
NRmatrix<T> NRmatrix<T>::operator*(const double &a)
// postcondition: normal assignment via copying has been performed;
//		if matrix and rhs were different sizes, matrix
//		has been resized to match the size of rhs
{
...
#ifdef _CheckDim_
if (mm!=rhs.ncols()||nn!=rhs.nrows()) {
	throw("Cannot perform deduction");
}
#endif
